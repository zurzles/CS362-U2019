...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
		return drewMineCard(choice2, choice1, currentPlayer, handPos, state);
    ...
    case baron:
		return drewBaronCard(choice1, currentPlayer, state);
    ...
    case tribute:
		return drewTributeCard(currentPlayer, nextPlayer, state);

    case ambassador:
		return drewAmbassadorCard(choice1, choice2, handPos, currentPlayer, state);
	...
	if (choice1 == BARON_DISCARD_ESTATE_CARD) { //Boolean true or going to discard an estate
		...
		while (card_not_discarded) {
			if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
				...
				for (int i = p; i < state->handCount[currentPlayer]; i++) {
...
int drewAmbassadorCard(int choice1, int choice2, int handPos, int currentPlayer, struct gameState* state) {
	...
	if (!(AMBASSADOR_CARD_RETURN_COUNT_MIN < choice2 < AMBASSADOR_CARD_RETURN_COUNT_MAX)){ //invalid choice2 selection
		return -1;
	}

	if (choice1 == handPos){ //cannot select current ambassador card as choice1
		return -1;
	}

	for (int i = 0; i < state->handCount[currentPlayer]; i++) {
		if (i != handPos && handCard(choice1, state) == state->hand[currentPlayer][i]){
			j++;
		}
	}
	if (j < choice2) //if do not have enough cards of desired discard count, return error
	{
		return -1;
	}
...
int drewMineCard(int choice1, int choice2, int currentPlayer, int handPos, struct gameState* state) {
	...
	if (!(j == copper || j ==silver || j == gold)) //invalid selection
	{
		return -1;
	}

	if (choice2 > treasure_map || choice2 < curse) //out of range
	{
		return -1;
	}

	if ((getCost(j) + 3) > getCost(choice2)) //if choice2 is greater than choice1
	{
		return -1;
	}
